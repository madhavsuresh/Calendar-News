...
class Bing():
  ...
  def getTwitter(self,txtReq):
    ...
    for result in result['SearchResponse']['Web']['Results']:
      ...
      if match:
        twit =  match.group(1)
        url = '%s/statuses/user_timeline.json?screen_name=%s&exclude_replies=true&count=%d' % (twit_base, twit,5)
        tweets = json.loads(urllib.urlopen(url).read());
        return {'handle' : twit, 'url':result['Url'],'tweets':tweets}
    return None
...
class Sentinal():
    ...
    def getNews(self,company):
        ...
        if symbol is None:
            ...
            headline_news = self.yahoo.getPublicNews(symbol,'headline',3)
            industry_news = self.yahoo.getPublicNews(symbol,'industry',3)
            ...
            stock_graph = ('http://chart.finance.yahoo.com/z?s=%s&'
                             'lang=en-US&region=US&width=350&height=180&l=3w') % (symbol)
